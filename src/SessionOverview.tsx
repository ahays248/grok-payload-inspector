...
export function SessionOverview({
  ...
  const callBreakdown = stats.series
    .map((p) => formatTokens(p.totals.total))
    .join(" + ");
  ...
  return (
    <section className="space-y-3">
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="rounded-md border border-line bg-panel px-3 py-2.5">
          <div className="text-[11px] uppercase tracking-wide text-muted">
            Latest request window
          </div>
          <div className="mt-1 font-mono text-2xl font-semibold tabular-nums">
            {formatTokens(stats.latestTokens)}
          </div>
          <div className="text-[11px] text-muted">
            One POST. /context is this, not the sum.
          </div>
        </div>
      </div>
      ...
      <p className="text-[11px] text-muted">
        Grok often bills 2–3 API calls for one Enter (reply, session title, dashboard
        line). You pay for each; the context bar is only the last one. Tools/MCP/skills
        should stay flat. Messages should climb until compact.
      </p>
    </section>
  );
}
